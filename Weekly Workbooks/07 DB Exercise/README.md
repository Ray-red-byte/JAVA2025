...
This workbook will lead you through the implementation of your first really significant Java program. The aim of this exercise is to build a database server (from the ground up !).
This will not only give you more practice writing complex Java programmes,
as well as providing you with hands-on experience of using query languages.  
...
In this exercise, you will build a relational database server from scratch.
...
to help get you started with this exercise.
...
It's going to be very difficult for your server to pass the tests if it has crashed !
...
In this exercise, you must use the file system for this purpose.
...
This might seem like a problematic limitation in the query language, however we have to draw the line somewhere (otherwise the exercise would become too complex to implement in the given timeframe).
...
If you don't know what normalisation is, then don't worry - you don't need to know for this exercise anyway !
...
It is not the aim of this exercise to address the topic of network or socket programming.
...
Automated testing scripts are typically designed to monitor what is returned via the network - these will not be able to pick up any `println` messages in the terminal !
...
will not be executed by the testing scripts.
...
The query language we will use for this exercise supports the following queries:
...
**Hints & Tips:**  
Note that you should NOT use any existing parsers or parser generators (Yacc, Lex, Antlr etc.) The aim of this exercise is to implement the command parsing using your own code.
...
In this exercise, we will not be providing you with an extensive set of tests cases for you to check the effectiveness of your code. Learning to write your own comprehensive set of test cases is a key part of becoming a 'developer' (if fact, this ability will be assessed in the final coursework assignment). The aim of the query transcript and the BNF grammar is to provide you with as much help as possible in achieving this objective. 
...
When you start to dig deeper into the complexities of this exercise, you will no doubt have a variety of questions of specific aspects of system functionality.
